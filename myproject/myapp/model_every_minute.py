...
def start_training():
    ...
    curl_command = [
        ...
        '--header',
        'Accept: text/plain',
        'http://localhost:5000/trade/download-trade-detail/2024-01-18/alL'
    ]
    ...
    max_sequence_length = max([len(features) for features in all_features])  # Example to use the max length
    all_features_padded = pad_sequences(all_features, maxlen=max_sequence_length, padding='post', dtype='float32')

    # Now, convert your padded feature list to a NumPy array
    X = np.array(all_features_padded)
...
def predict_unseen_data():
    ...
    curl_command_unseen = [
        ...
        '--header',
        'Accept: text/plain',
        'http://localhost:5000/trade/download-active-trade-detail-unseen'
    ]
