...
def train_model():
    ...
    url = 'http://localhost:5000/trade/download-every-minute-training-data-v1-pe'
    ...
    target_list = []
    for record in data:
        features = [record['indiaVix'], record['entry'], record['theta'], record['oi_change'], record['max_oi'],
                    record['ltp_change'],
    ...
    model.save('model_every_minute_pe.keras')
...
def predict_unseen(raw_unseen_data):
    ...
    model = tf.keras.models.load_model('model_every_minute_pe.keras')
